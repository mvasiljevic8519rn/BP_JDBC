...
public class MYSQLrepository implements Repository{
    ...
    private Map<String,ArrayList<String>>databaseTables;
...
//                    }
                ...
                databaseTables.put(tableName,pom);
    ...
    @Override
    public List<Row> set(String from){
	...
	@Override
	public List<Row> update(String from) {
		List<Row> rows = new ArrayList<>();
        try{
            ...
            PreparedStatement preparedStatement = connection.prepareStatement(from);
            preparedStatement.executeUpdate();
            
            String []pom=from.split("into ");
            String []pom1=pom[1].split(" ");
            
            preparedStatement = connection.prepareStatement("select * from "+pom1[0]);
            ResultSet rs=preparedStatement.executeQuery();
            while (rs.next()){
                Row row = new Row();
                row.setName(from);
